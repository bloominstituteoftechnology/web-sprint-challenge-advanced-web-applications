...
1. Explain what a token is used for. A token is used to give users to content that is protected on a webpage.
2. What steps can you take in your web apps to keep your data secure? Make the password length requirement longer and and maybe even a two step authentication.
3. Describe how web servers work. Web servrs store the code for websites. When you go to a webpage, the host server sends the code to the api which then send it to the screen.
4. Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers. GET POST PUT and DELETE
